...
static void inbox_received(DictionaryIterator *iter, void *context) {
  ...
  if (s_buffer == NULL) {
    ...
    if (!s_buffer) {
      if (s_on_error) s_on_error(ERR_OUT_OF_MEMORY);
      return;
    }
    s_buffer[0] = '\0';
  ...
  // Null-terminate at offset+len; will be overwritten by later chunks.
  ...
  if (s_received_mask == complete_mask) {
    // Compute true total length to null-terminate cleanly.
...
void transport_init(TransportResponseHandler on_response,
                    TransportErrorHandler on_error) {
